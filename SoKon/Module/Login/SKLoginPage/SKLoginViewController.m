...
{
    SKLog(@"%@",[NSString stringWithFormat:@"点击了第三方登录按钮%ld",(long)sender.tag]);
    NSInteger num = sender.tag;
    SKThirdManager *manager = [SKThirdManager manager];
    if (num == 10) {
        [manager setQQPlatformForLoginWithPresentingController:self];
    }else if (num == 11) {
        [manager setWXPlatformForLoginWithPresentingController:self];
    }else if (num == 12) {
        [manager setSinaPlatformForLoginWithPresentingController:self];
    }else{
        [manager requestUnOauthWithType:UMShareToSina];
    }
}
