...
- (void)setBadgeValue:(NSString *)badgeValue
{
    ...
    if ([badgeValue isEqualToString:@"new"]) {
        ...
        NSInteger num = [badgeValue integerValue];
                  num = num > 99 ? 99 : num;
        if (num < 10 && num > 0) {
            [self setBadgeItemWithNumber:0 badgeValue:[NSString stringWithFormat:@"%ld",(long)num]];
        }else if (10 <num && num <= 99) {
            [self setBadgeItemWithNumber:2 badgeValue:[NSString stringWithFormat:@"%ld",(long)num]];
        }
    }
}
